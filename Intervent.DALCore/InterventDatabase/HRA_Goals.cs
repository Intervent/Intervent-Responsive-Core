namespace Intervent.DAL
{
    using System.ComponentModel.DataAnnotations;
    using System.ComponentModel.DataAnnotations.Schema;

    public partial class HRA_Goals
    {
        [DatabaseGenerated(DatabaseGeneratedOption.None)]
        public int Id { get; set; }

        public float? StWt { get; set; }

        public float? LtWt { get; set; }

        public short? StExPt { get; set; }

        public short? LtExPt { get; set; }

        public bool ASCVD { get; set; }

        public bool Diabetes { get; set; }

        public float? LtLdl { get; set; }

        public float? OptimalLdl { get; set; }

        public bool ReduceLdl { get; set; }

        public float? LtHdl { get; set; }

        public bool RaiseHDL { get; set; }

        public float? LtTrig { get; set; }

        public bool ReduceTrig { get; set; }

        public short? LtSBP { get; set; }

        public short? LtDBP { get; set; }

        public bool ReduceBP { get; set; }

        public float? LtGluc1 { get; set; }

        public float? LtGluc2 { get; set; }

        public bool ReduceGluc { get; set; }

        public float? LtA1c { get; set; }

        public float? TenYrProb { get; set; }

        public float? TenYrAvg { get; set; }

        public float? TenYrLow { get; set; }

        public byte MetSyn { get; set; }

        [StringLength(5)]
        public string? MetSynDets { get; set; }

        [StringLength(1)]
        public string? ExerPlan { get; set; }

        [StringLength(1)]
        public string? NutMealPlan { get; set; }

        public bool NutMaint { get; set; }

        [StringLength(1)]
        public string? NutMaintMealPlan { get; set; }

        public bool NutLowBMI { get; set; }

        [StringLength(1)]
        public string? NutLowBMIMealPlan { get; set; }

        public short? NutFatNum { get; set; }

        public bool NutLtFat { get; set; }

        public short? NutLtFatNum { get; set; }

        public bool NutUnderWeight { get; set; }

        public short? NutSatFatNum { get; set; }

        public bool NutLtSatFat { get; set; }

        public short? NutLtSatFatNum { get; set; }

        public short? NutCholNum { get; set; }

        public bool NutDrinkWater { get; set; }

        public short? NutDrinkWaterNum { get; set; }

        public short? NutSugNum { get; set; }

        public float? NutStcal { get; set; }

        public float? NutLtcal { get; set; }

        public bool DrClearRef1 { get; set; }

        public bool DrClearRef2 { get; set; }

        public bool BloodTestRef { get; set; }

        public bool MedRef1 { get; set; }

        public bool MedRef2 { get; set; }

        public bool LdlRef1 { get; set; }

        public bool LdlRef2 { get; set; }

        public bool LdlRef3 { get; set; }

        public bool LdlRef4 { get; set; }

        public float? LdlRef4Num { get; set; }

        public bool LdlRef5 { get; set; }

        public bool LdlRef6 { get; set; }

        public bool LdlRef7 { get; set; }

        public bool HdlRef1 { get; set; }

        public bool HdlRef2 { get; set; }

        public bool TrigRef1 { get; set; }

        public bool TrigRef2 { get; set; }

        public bool TrigRef3 { get; set; }

        public bool TrigRef4 { get; set; }

        public bool TrigRef5 { get; set; }

        public bool MedRef3 { get; set; }

        public bool CholRef1 { get; set; }

        public bool CholRef2 { get; set; }

        public bool GlucRef1 { get; set; }

        public bool GlucRef2 { get; set; }

        public bool GlucRef3 { get; set; }

        public bool A1CRef1 { get; set; }

        public bool A1CRef2 { get; set; }

        public bool AspRef { get; set; }

        public bool NicRef { get; set; }

        public bool BPRef { get; set; }

        public bool RMRRef { get; set; }

        public float? TenYearASCVD { get; set; }

        public float? TenYearASCVDGoal { get; set; }

        public float? LifetimeASCVD { get; set; }

        public float? LifetimeASCVDGoal { get; set; }

        public int? ASCVDRef { get; set; }

        public bool? HypertensiveBPRef { get; set; }

        public virtual HRA HRA { get; set; }
    }
}
