    ...
    public class ReportsController : BaseController
    {
        ...
        public async Task<ActionResult> PrintableReportsAsync(string view, bool generatePDF, bool? sendFax, string faxNumber, int? awvReportId, int? kitId, string pageIdentifier, int? kitsInUserProgramsId, string languageCode, string kitColor)
        {
            ...
            if (view == "KitStepDetails")
            {
                ...
                //viewResult = ViewEngines.Engines.FindPartialView(ControllerContext, "../Kit/_KitStepDetails");
            }
            //else
            //viewResult = ViewEngines.Engines.FindView(ControllerContext, view, null);
            //ViewContext viewContext = new ViewContext(ControllerContext, viewResult.View, viewData, new TempDataDictionary(), stringWriter);
            // viewResult.View.Render(viewContext, stringWriter);
            string baseUrl;
            if (generatePDF || (sendFax.HasValue && sendFax.Value))
                baseUrl = string.Format("{0}://{1}{2}", Request.Scheme, Request.Host.Value, Url.Content("~"));
            else
            {
                string currentPageUrl = Request.Headers["Referer"].ToString();
                string path = "Reports/PrintableReports?view=" + view + "&generatePDF=" + generatePDF;
                baseUrl = currentPageUrl.Substring(0, currentPageUrl.Length - path.Length);
            }
