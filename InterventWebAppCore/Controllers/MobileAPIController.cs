...
namespace InterventWebApp.Controllers
{
	...
	public class MobileAPIController : ControllerBase
	{
			...
			if (userIdentity != null && userIdentity.UserId != 0)
			{
				...
				if (Request.Form.Files.Count > 0)
				{
					...
					if (postedFileExtension.ToLower().Equals(".pdf") || postedFileExtension.ToLower().Equals(".jpg") || postedFileExtension.ToLower().Equals(".png") || postedFileExtension.ToLower().Equals(".jpeg") || postedFileExtension.ToLower().Equals(".gif"))
					{
						...
						request.attachement_name = fileName;
				...
				{
					if (data.Key == "parent_message_id")
					{
						request.parent_message_id = Convert.ToInt32(data.Value);
					}
					else if (data.Key == "message_id")
					{
						request.message_id = Convert.ToInt32(data.Value);
					}
					else if (data.Key == "subject")
					{
						request.subject = data.Value;
					}
					else if (data.Key == "message")
					{
						request.message = data.Value;
					}
					else if (data.Key == "is_sent")
					{
						request.is_sent = Convert.ToBoolean(data.Value);
					}
				}
		...
		public async Task<IActionResult> UploadProfilePicture()
		{
			...
			try
			{
				if (userIdentity != null && userIdentity.UserId != 0 && Request.Form.Files.Count > 0)
				{
					...
					{
						IFormFile postedFile = Request.Form.Files["attachment"];
						if (postedFile != null && await CheckIfImageAsync(postedFile))
						{
							var fileName = DateTime.Now.ToString("_ddMMyyhhmmssFFF") + Path.GetExtension(postedFile.FileName);
							using (FileStream stream = new FileStream(Path.Combine(filePath, fileName), FileMode.Create))
							{
								postedFile.CopyTo(stream);
							}
							await AccountUtility.UploadPicture(_userManager, userIdentity.UserId, fileName, filePath);
							return Ok(new { status = true, message = "Successfully updated" });
						}
					}
					return BadRequest("Invalid request");
		...
		private async Task<bool> CheckIfImageAsync(IFormFile postedFile)
		{
