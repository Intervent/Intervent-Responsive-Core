...
using ClaimDataAnalytics.Claims.Converters.CVSCaremark;
...
namespace ClaimDataAnalytics.Claims.Classmaps
{
    public sealed class CVSCaremarkClassMap : CsvClassMap<ClaimsInputFlatFileModel>
    {
        public CVSCaremarkClassMap()
        {
            ...
            Map(m => m.Copay).Index(222).TypeConverter(new AmountConverter());//,[Amount of Copay]
            Map(m => m.Coinsurance).Index(223).TypeConverter(new AmountConverter());//,[Amount of Coinsurance]
            Map(m => m.NetPaid).Index(257).TypeConverter(new AmountConverter());//,[Net Amount Due (Total Amount Billed-Paid)]
