...
using ClaimDataAnalytics.Claims.Converters.AETNA;
...
namespace ClaimDataAnalytics.Claims.Classmaps
{
    public sealed class AetnaFileClassMap : CsvClassMap<ClaimsInputFlatFileModel>
    {
        public AetnaFileClassMap()
        {
            ...
            Map(m => m.SubscriberSSN).Index(16).TypeConverter(new SSNConverter());//[Employee's SSN]
            ...
            Map(m => m.ServiceEndDate).Index(60).TypeConverter(new DateFormatConverter("yyyy-MM-dd"));//	,[Date Service Stopped]
