...
using ClaimDataAnalytics.Claims.Converters.BCBS;
...
namespace ClaimDataAnalytics.Claims.Classmaps
{
     public sealed class BCBSFileClassMap : CsvClassMap<ClaimsInputFlatFileModel>
    {
        ...
        public BCBSFileClassMap()
        {
            ...
            Map(m => m.SubscriberSSN).Index(1);//ltrim(rtrim(isnull([SUBSCRIBER_NUMBER],'''')))
            ...
            Map(m => m.ServiceEndDate).Index(53).TypeConverter(new DateFormatConverter(dateFormat)); //,[Service End Date]
            ...
            Map(m => m.AllowedAmount).Index(64);//,[ALLOWED AMOUNT]
